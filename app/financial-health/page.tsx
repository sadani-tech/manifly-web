...
const LABELS: Record<string, string> = { savingsRate: "Porsi tabungan", budgetAdherence: "Kesesuaian rencana", cashflowStability: "Kestabilan arus uang", debtPressure: "Tekanan utang", dataCompleteness: "Kelengkapan catatan" };
...
export default function FinancialHealthPage() {
  ...
  useEffect(() => { void load(); }, []);
  if (loading) return <p className="text-sm text-muted-foreground">Manifly sedang memeriksa jejak uangmu…</p>;
  if (!health?.enabled) return <div className="space-y-4"><h2 className="text-3xl font-black">Financial Health</h2><p className="text-muted-foreground">Aktifkan saat kamu siap melihat gambaran dan langkah yang bisa dicoba.</p><Button onClick={async () => { await financialHealthApi.setEnabled(true); await load(); }}>Lihat kondisiku</Button></div>;
  ...
  const comparisonText = health.comparison
    ...
    : null;
  return <div className="space-y-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-kicker">Jejak uangmu</p><h2 className="mt-1 text-3xl font-black">Kondisi keuangan</h2><p className="text-sm text-muted-foreground">Gambaran periode {health.period} · perhitungan {health.formulaVersion}</p></div><Button variant="outline" onClick={load}><RefreshCw className="h-4 w-4" /> Perbarui gambaran</Button></div>
    ...
    <Button variant="ghost" onClick={async () => { await financialHealthApi.setEnabled(false); await load(); }}>Sembunyikan kondisi</Button>
  </div>;
}
